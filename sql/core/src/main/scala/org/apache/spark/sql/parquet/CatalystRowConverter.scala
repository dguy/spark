...
private[parquet] class CatalystRowConverter(
  ...
  private val needsArrayReset = fieldConverters.filter(converter => converter.isInstanceOf[NeedsResetArray])
    ...
    catalystType match {
      ...
      case t: ArrayType => {
        parquetType.isRepetition(Type.Repetition.REPEATED) match {
          case true => {
            t match {
              case ArrayType(elementType:StructType, _) => new CatalystRepeatedStructConverter(parquetType.asGroupType(), elementType, updater)
              case ArrayType(elementType:DataType,_) => new CatalystRepeatedPrimitiveConverter(parquetType,elementType,updater)
            }
          }
          case false => new CatalystArrayConverter(parquetType.asGroupType(), t, updater)
        }
      }
  ...
  private final class CatalystRepeatedPrimitiveConverter(
    ...
    override def addValueFromDictionary(dictionaryId: Int): Unit = stringConverter.addValueFromDictionary(dictionaryId)

    override def setDictionary(dictionary: Dictionary): Unit = stringConverter.setDictionary(dictionary)
  ...
  private class CatalystRepeatedStructConverter(groupType: GroupType, structType: StructType, updater: ParentContainerUpdater)
   extends CatalystRowConverter(groupType, structType, updater) with NeedsResetArray{
